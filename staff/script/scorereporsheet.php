...
session_start();
//set_time_limit(0);
$selectquery = "select * from student_tab where student_id='".$_POST["newdata"]."' and clientapp_id='".$_SESSION['clientappid']."'";
						...
						echo '	<div class="table-responsive"><table width="75%" class="" style="width:75%">	
							...
							<tbody>
								<tr>
									...
									<td width="38%">Session:<strong>'.$_POST["stdsession"].' Academic Year</strong></td>
								</tr>
								<tr>
									<td>Admission Year:  <strong>'.$_SESSION['admyear'].'</strong></td>
									<td>Term: <strong>'.$_POST["stdterm"].'</strong></td>
								</tr>
								<tr>
									<td>Class:  <strong>'.$_SESSION['cclass']." ".$rows['current_arm'].'</strong></td>
...
if(isset ($_SESSION['cclass']) && isset ($_POST["newdata"]))
{	
$stdclass=$_SESSION['cclass'];
$examsession=$_POST["stdsession"];
$examterm=$_POST["stdterm"];
if ($examterm=="First")
{
	 
$id=$_POST["newdata"];
//$squery="SELECT `student_tab.student_id` AS 'ID', SUM(CASE WHEN `exam_type` = 'FirstCA' THEN `mark` ELSE '0' END) AS 'FirstTest',SUM(CASE WHEN `exam_type` = 'SecondCA' THEN `mark` ELSE '0' END) AS 'SecondTest', SUM(`mark`) as 'Total',SUM(CASE WHEN `exam_type` = 'termexam' THEN `mark` ELSE '0' END) AS 'Examscore', SUM(`mark`) as 'Total' FROM scoresheet_tab where `student_class` = 'Junior Secondary School 1' GROUP BY `student_id`";
$squery="SELECT exam_subject AS 'Subject', SUM(CASE WHEN exam_type = 'FirstCA' and exam_term='First' THEN mark ELSE '0' END) AS 'FirstTest',SUM(CASE WHEN exam_type = 'SecondCA' and exam_term='First' THEN mark ELSE '0' END) AS 'SecondTest',SUM(CASE WHEN exam_type = 'termexam' and exam_term='First' THEN mark ELSE '0' END) AS 'Examscore', SUM(mark) as 'Total' FROM scoresheet_tab where student_class ='$stdclass' and student_id = '$id' and exam_session='$examsession' and exam_term='$examterm' and clientapp_id='".$_SESSION['clientappid']."' GROUP BY exam_subject" ;
$result=mysqli_query($con,$squery);
		if(mysqli_num_rows($result))
		{
			...
			echo '</tbody>
				...
				<tr align="center"><td colspan="15"><strong><span class="pull-left"><u><img src="../'.$_SESSION['clientsign'].'"></u><br>Authorized Signature</span></strong><strong><span class="pull-right"><u>'.$_SESSION['clienttend'].'</u><br>Date</span></strong></td>
				</tr>
			</table></div>';
		}
		else {
...
SUM(CASE WHEN exam_type = 'SecondCA' and exam_term='First' THEN mark ELSE '0' END) AS 'FirstTermCA2',SUM(CASE WHEN exam_type = 'termexam' and exam_term='First' THEN mark ELSE '0' END) AS 'FirstExamscore',SUM(CASE WHEN exam_term='First' THEN mark ELSE '0' END)  as 'FirstTotal', SUM(CASE WHEN exam_type = 'FirstCA' and exam_term='Second' THEN mark ELSE '0' END) AS 'SedcondTermCA1',SUM(CASE WHEN exam_type = 'SecondCA' and exam_term='Second' THEN mark ELSE '0' END) AS 'SedcondTermCA2', SUM(CASE WHEN exam_type = 'termexam' and exam_term='Second' THEN mark ELSE '0' END) AS 'SecondExamscore', SUM(CASE WHEN exam_term='Second' THEN mark ELSE '0' END)  as 'SecondTotal' FROM scoresheet_tab where student_class ='$stdclass' and student_id = '$id' and exam_session='$examsession' and clientapp_id='".$_SESSION['clientappid']."' Group By exam_subject" ;
$result=mysqli_query($con,$squery);
		if(mysqli_num_rows($result))
		{
			$count=mysqli_num_rows($result);
